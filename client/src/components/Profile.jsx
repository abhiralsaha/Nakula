...
const Profile = () => {
    ...
    return (
        <div className="max-w-4xl mx-auto space-y-6 pb-20">
            ...
            <div className={cardClass}>
                <h2 className="text-xl font-bold mb-4 flex items-center gap-2">
                    <Smartphone className={isDark ? 'text-indigo-400' : 'text-indigo-600'} />
                    Mobile Verification
                </h2>
                <p className={`text-sm mb-4 ${isDark ? 'text-zinc-400' : 'text-gray-600'}`}>
                    Link your phone number to receive SMS notifications.
                </p>
                ...
                <div className="flex flex-col gap-4 max-w-md">
                    <div className="flex gap-2">
                        <input
                            ...
                            className={`flex-1 p-3 rounded-xl border outline-none ${isDark
                                    ? 'bg-black border-zinc-700 text-white focus:border-indigo-500'
                                    : 'bg-gray-50 border-gray-300 text-gray-900 focus:border-indigo-500'
                                }`}
                        />
                        {!userData?.isMobileVerified && !showOtpInput && (
                            <button
                        ...
                        {userData?.isMobileVerified && (
                            <div className="flex items-center gap-2 text-green-500 px-3 font-medium">
            ...
            <div className={cardClass}>
                ...
                <p className={`text-sm mb-6 ${isDark ? 'text-zinc-400' : 'text-gray-600'}`}>
                    Choose how you want to be notified for tasks.
                </p>

                <div className="space-y-4">
                    <Toggle
                        label="SMS Notifications"
                        description="Receive task reminders via text message on your verified number."
                        active={preferences.sms}
                        onChange={() => togglePreference('sms')}
                        isDark={isDark}
                        disabled={!userData?.mobileNumber || !userData?.isMobileVerified}
                    />
                    <Toggle
                    ...
                    />
                    <Toggle
                        label="App Alarm / Push"
                        description="Get in-app alerts and browser notifications."
                        active={preferences.alarm}
                        onChange={() => togglePreference('alarm')}
                        isDark={isDark}
                    />
                </div>

                {preferences.sms && (!userData?.mobileNumber || !userData?.isMobileVerified) && (
                    <div className={`mt-4 p-3 rounded-xl flex items-center gap-2 text-sm ${isDark ? 'bg-yellow-900/20 text-yellow-500' : 'bg-yellow-50 text-yellow-700'}`}>
                        <AlertTriangle size={16} />
                        Please verify your mobile number to enable SMS notifications.
                    </div>
                )}
            </div>
        </div>
